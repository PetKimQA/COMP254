...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  /**
   * Returns the zero-based index of Position p in the list,
   * or –1 if p is null or not found.
   */
  @Override
  public int indexOf(Position<E> p) {
    // immediately handle null
    if (p == null) return -1;

    // ensure p is one of our nodes (or throw if it’s invalid)
    Node<E> target = validate(p);

    // walk from first real node, counting positions
    int index = 0;
